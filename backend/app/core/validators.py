...
class ProgramacionValidator:
    ...
    def validate_programacion(
        ...
        if sala_conflicto:
            errors.append(
                ...
                f"(Caso: {sala_conflicto.solicitud_id})"
            )
        ...
        if perito_conflicto:
            errors.append(
                ...
                f"(Caso: {perito_conflicto.solicitud_id})"
            )
    ...
    def _check_conflicto_sala(
        ...
        # Query para buscar programaciones conflictivas
        statement = select(Programacion).where(
            and_(
                ...
                or_(
                    ...
                    and_(
                        Programacion.fecha_hora <= fecha_hora,
                        Programacion.fecha_hora + timedelta(minutes=Programacion.duracion_minutos) > fecha_hora
                    ),
                    # Caso 2: Nueva cita termina durante una cita existente
                    and_(
                        Programacion.fecha_hora < fecha_fin,
                        Programacion.fecha_hora + timedelta(minutes=Programacion.duracion_minutos) >= fecha_fin
                    ),
                    # Caso 3: Nueva cita envuelve completamente a una cita existente
                    and_(
                        Programacion.fecha_hora >= fecha_hora,
                        Programacion.fecha_hora + timedelta(minutes=Programacion.duracion_minutos) <= fecha_fin
                    )
        ...
        conflicto = self.session.exec(statement).first()
        return conflicto
