...
async def list_sedes(
    ...
    """Lista todas las sedes (filtradas por distrito si no es admin)"""
    statement = select(Sede)

    # Si no es admin, solo mostrar sedes de su distrito
    if current_user.role != UserRole.ADMIN:
        ...
        statement = statement.where(Sede.distrito_fiscal_id == current_user.distrito_fiscal_id)
...
async def get_sede(
    ...
    """Obtiene una sede por ID (validando acceso por distrito)"""
    sede = session.get(Sede, sede_id)
    if not sede:
        raise HTTPException(status_code=404, detail="Sede not found")

    # Si no es admin, validar que la sede pertenezca a su distrito
    if current_user.role != UserRole.ADMIN:
        if current_user.distrito_fiscal_id is None:
            raise HTTPException(
                status_code=403,
        ...
        if sede.distrito_fiscal_id != current_user.distrito_fiscal_id:
            raise HTTPException(
                status_code=403,
                detail="No tiene permiso para acceder a esta sede"
            )
...
async def update_sede(
    ...
    sede.updated_at = datetime.now(timezone.utc)
    session.add(sede)
    session.commit()
    session.refresh(sede)
    return sede
...
async def search_sedes_advanced(
    ...
    (filtradas por distrito si no es admin)
    """
    ...
    # FILTRO POR DISTRITO: Si no es admin, solo mostrar sedes de su distrito
    if current_user.role != UserRole.ADMIN:
        if current_user.distrito_fiscal_id is None:
            raise HTTPException(
                status_code=403,
