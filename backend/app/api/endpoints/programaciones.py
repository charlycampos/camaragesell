...
async def list_programaciones(
    ...
    """Lista programaciones según el rol del usuario (filtradas por distrito si no es admin)"""
    ...
    # Si no es admin, filtrar por distrito fiscal a través de Solicitud -> DespachoFiscal
    if current_user.role != UserRole.ADMIN:
        ...
        # Join con Solicitud y DespachoFiscal para filtrar por distrito
        statement = statement.join(Solicitud).join(DespachoFiscal).where(
...
async def create_programacion(
    ...
    Dos casos de uso:
    1. Con solicitud_id: Aprobar/programar una solicitud pendiente de un fiscal
    2. Sin solicitud_id: Programación directa (ej: llamada telefónica)
    """
    ...
    # Si hay solicitud_id, verificar que existe y está pendiente
    solicitud = None
    if programacion_data.solicitud_id:
    ...
    # Si hay solicitud, actualizar su estado
    if solicitud:
...
async def update_programacion(
    ...
    programacion.updated_at = datetime.now(timezone.utc)
    session.add(programacion)
    session.commit()
    session.refresh(programacion)
    return programacion
...
async def list_programaciones_enriched(
    ...
    Incluye información completa de sala, perito, solicitud, etc. (filtradas por distrito)
    """
    statement = select(Programacion)
...
async def search_programaciones_advanced(
    ...
    # FILTRO POR DISTRITO: Si no es admin, solo mostrar programaciones de su distrito
    if current_user.role != UserRole.ADMIN:
        if current_user.distrito_fiscal_id is None:
            raise HTTPException(
                status_code=403,
