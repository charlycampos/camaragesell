...
async def list_salas(
    ...
    """Lista todas las salas del distrito fiscal del usuario"""
    # Filtrar por distrito fiscal del usuario
    if current_user.role == UserRole.ADMIN:
        # Admin ve todas las salas
        statement = select(Sala).offset(skip).limit(limit)
    else:
        # Otros usuarios solo ven salas de su distrito fiscal
        statement = (
            select(Sala)
            .join(Sede)
            .where(Sede.distrito_fiscal_id == current_user.distrito_fiscal_id)
            .offset(skip)
            .limit(limit)
        )
...
async def search_salas_advanced(
    ...
    # Filtrar por distrito fiscal (excepto admin)
    if current_user.role != UserRole.ADMIN:
        ...
        statement = statement.join(Sede).where(
            Sede.distrito_fiscal_id == current_user.distrito_fiscal_id
        )
