...
async def get_resumen_general(
    ...
    solicitudes_programadas = session.exec(
        select(func.count(Solicitud.id))
        .where(Solicitud.estado == EstadoSolicitud.PROGRAMADA)
    ).one()
    ...
    total_salas = session.exec(select(func.count(Sala.id)).where(Sala.is_active == True)).one()
    total_peritos = session.exec(select(func.count(Perito.id)).where(Perito.is_active == True)).one()

    return {
        "total_solicitudes": total_solicitudes,
        "solicitudes_pendientes": solicitudes_pendientes,
        "solicitudes_programadas": solicitudes_programadas,
...
async def get_solicitudes_por_mes(
    ...
    for solicitud in solicitudes:
        ...
        elif solicitud.estado == EstadoSolicitud.PROGRAMADA:
            meses_dict[mes_key]["programadas"] += 1
...
async def get_programaciones_por_perito(
    ...
    peritos = session.exec(select(Perito).where(Perito.is_active == True)).all()
...
async def get_ocupacion_salas(
    ...
    salas = session.exec(select(Sala).where(Sala.is_active == True)).all()
...
async def get_estadisticas_fiscal(
    ...
    # Tasa de programación (solicitudes programadas sobre el total)
    tasa_programacion = 0
    if total_solicitudes > 0:
        tasa_programacion = round((programadas / total_solicitudes) * 100, 2)
    ...
    return {
        ...
        "tasa_programacion": tasa_programacion
    }
